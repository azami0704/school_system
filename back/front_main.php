    ...
    <ul class="pagination">
            ...
            if($Pages>$pagiRange){
                if($pageActive > 5 && $Pages-$pageActive>4){
                    ...
                    $pagiFirstPage = "<li><a href='?pageSet={$pageSetting}&do=$do&page=1'>1</a></li>...";
                    $pagiLastPage = "...<li><a href='?pageSet={$pageSetting}&do=$do&page={$Pages}'>{$Pages}</a></li>";
            ...
            //頁數輸出
            echo "<li $preDisable><a href='?pageSet={$pageSetting}&do=$do&page={$prevPage}{$codeForPageChange}'><</a></li>";
            echo $pagiFirstPage;
            for($i=$pagiStart;$i<=$pageEnd;$i++){
                if($i==$pageActive){
                    echo "<li class='active'><a href='?pageSet={$pageSetting}&do=$do&page={$i}{$codeForPageChange}'>{$i}</a></li>";
                }else{
                    echo "<li><a href='?pageSet={$pageSetting}&do=$do&page={$i}{$codeForPageChange}'>{$i}</a></li>";
                }
            }
            echo $pagiLastPage;
            echo "<li $nextDisable><a href='?pageSet={$pageSetting}&do=$do&page={$nextPage}{$codeForPageChange}'>></a></li>";
            //頁數輸出 END
            ?>
        </ul>
</div>