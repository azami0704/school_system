    ...
    <div class="page-list">
            ...
            <script>
                ...
                pageSet.addEventListener('change',function(e){
                    ...
                    if(url){
                        location.href=`?do=main&pageSet=${e.target.value}&code=${url}`;
                    }else{
                        location.href=`?do=main&pageSet=${e.target.value}`;
                    }
                })
            </script>
    </div>
    <ul class="pagination">
            ...
            if($Pages>$pagiRange){
                if($pageActive > 5 && $Pages-$pageActive>4){
                    ...
                    $pagiFirstPage = "<li><a href='?do=main&pageSet={$pageSetting}&page=1'>1</a></li>...";
                    $pagiLastPage = "...<li><a href='?do=main&pageSet={$pageSetting}&page={$Pages}'>{$Pages}</a></li>";
            ...
            //頁數輸出
            echo "<li $preDisable><a href='?do=main&pageSet={$pageSetting}&page={$prevPage}{$codeForPageChange}'><</a></li>";
            echo $pagiFirstPage;
            for($i=$pagiStart;$i<=$pageEnd;$i++){
                if($i==$pageActive){
                    echo "<li class='active'><a href='?do=main&pageSet={$pageSetting}&page={$i}{$codeForPageChange}'>{$i}</a></li>";
                }else{
                    echo "<li><a href='?do=main&pageSet={$pageSetting}&page={$i}{$codeForPageChange}'>{$i}</a></li>";
                }
            }
            echo $pagiLastPage;
            echo "<li $nextDisable><a href='?do=main&pageSet={$pageSetting}&page={$nextPage}{$codeForPageChange}'>></a></li>";
            //頁數輸出 END
            ?>
        </ul>
        </div>
